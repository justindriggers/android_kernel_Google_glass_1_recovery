...
//Must know which of these are needed
...
// modified from F 34, must check for copy-pasta nonsense
...
static ssize_t rmi_fn_54_data_read(struct kobject *kobj, struct bin_attribute *attributes,
...
void FN_54_inthandler(struct rmi_function_info *rmifninfo,
	...
	switch(instance_data->reporttype) {
		case r8bit_image:
			instance_data->reportsize = instance_data->numrxelectrodes * instance_data->numtxelectrodes;
			break;
		case r16bit_image:
		case autoscan:
			instance_data->reportsize = 2 * instance_data->numrxelectrodes * instance_data->numtxelectrodes;
			break;
		case trans2trans:
		case trans2trans_short:
			instance_data->reportsize = instance_data->numtxelectrodes * instance_data->numtxelectrodes;
			break;
		case rec2rec:
			instance_data->reportsize = instance_data->numrxelectrodes * instance_data->numrxelectrodes;
			break;
		default:
			instance_data->reportsize = 0;
			pr_err("%s : Invalid report type set. This should never happen. \n", __func__);
	}
	// We need to ensure the buffer is big enough. A Buffer size of 0 means that the buffer has not been allocated.
	if (instance_data->bufsize < instance_data->reportsize) {
		...
		instance_data->report_data = kzalloc(instance_data->reportsize, GFP_KERNEL);
	...
	if (!instance_data->report_data) {
		pr_err("%s: Error allocating memory for rmi_fn_54_data.report_data.\n",
		       __func__);
		instance_data->status = ENOMEM;
		return;
	}
	...
	/*for (i = 0; i < instance_data->reportsize; i++) {
		...
		*to_write = 'a';
		...
		if ( error ) {
			pr_err("%s: Error occured while reading data: Error %d\n",
		       __func__, error);
...
static ssize_t rmi_fn_54_reporttype_store(struct device *dev,
	...
	if (result != count) {
		if (result < 0) {
			...
			dev_err(dev, "%s : Unexpected number of lines written to 0x%x\n",
			       __func__, fn->rfi->function_descriptor.data_base_addr);
		...
		if (result != count) {
			if (result < 0) {
				dev_err(dev, "%s : Could not write command to 0x%x\n",
				       __func__, fn->rfi->function_descriptor.command_base_addr);
			} else {
				dev_err(dev, "%s : Unexpected number of lines written to 0x%x\n",
				       __func__, fn->rfi->function_descriptor.command_base_addr);
				dev_err(dev, "Wrote: %d\tExpected:%d\n", result, count);
...
static ssize_t rmi_fn_54_control_store(struct device *dev,
	...
	result =
	    rmi_write_multiple(fn->sensor, fn->rfi->function_descriptor.control_base_addr,
			       &data,1);
	if (result != count) {
		if (result < 0) {
			dev_err(dev, "%s : Could not write control to 0x%x\n",
			       __func__, fn->rfi->function_descriptor.control_base_addr);
		} else {
			dev_err(dev, "%s : Unexpected number of lines written to 0x%x\n",
			       __func__, fn->rfi->function_descriptor.control_base_addr);
			dev_err(dev, "Wrote: %d\tExpected:%d\n", result, count);
...
static ssize_t rmi_fn_54_fifoindexlo_store(struct device *dev,
	...
	result =
	    rmi_write_multiple(fn->sensor, fn->rfi->function_descriptor.data_base_addr + 1,
			       &data,1);
	if (result != count) {
		if (result < 0) {
			dev_err(dev, "%s : Could not write FIFOIndexLo to 0x%x\n",
			       __func__, fn->rfi->function_descriptor.data_base_addr + 1);
		} else {
			dev_err(dev, "%s : Unexpected number of lines written to 0x%x\n",
			       __func__, fn->rfi->function_descriptor.data_base_addr + 1);
			dev_err(dev, "Wrote: %d\tExpected:%d\n", result, count);
...
static ssize_t rmi_fn_54_fifoindexhi_store(struct device *dev,
	...
	data = (unsigned char)val;
	result =
	    rmi_write_multiple(fn->sensor, fn->rfi->function_descriptor.data_base_addr + 2,
			       &data,1);
	if (result != count) {
		if (result < 0) {
			dev_err(dev, "%s : Could not write FIFOIndexHi to 0x%x\n",
			       __func__, fn->rfi->function_descriptor.data_base_addr + 2);
		} else {
			dev_err(dev, "%s : Unexpected number of lines written to 0x%x\n",
			       __func__, fn->rfi->function_descriptor.data_base_addr + 2);
			dev_err(dev, "Wrote: %d\tExpected:%d\n", result, count);
