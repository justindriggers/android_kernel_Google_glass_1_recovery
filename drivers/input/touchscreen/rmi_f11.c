...
#define GESTURE_OFFSET_X 10000
#define GESTURE_OFFSET_Y 1000
#define GESTURE_MT_TRACKING 10
		      ...
		      unsigned int asserted_IRQs)
{
	...
	/* Only report events if we have not masked them. */
	if (!f11->mask_events) {
	...
	if (send_synth_key(f11, finger_down_count)) {
		...
		if (flag & HAS_FLICK_MASK) {
			...
			/* Use extreme tracking id to indicate it's not a real finger */
			input_report_abs(function_device->input, ABS_MT_TRACKING_ID, GESTURE_MT_TRACKING);
			...
			/* Send an empty sync packets with empty mt sync to indicate all fingers
			 * have left the touchpad. */
			input_mt_sync(function_device->input);
			input_sync(function_device->input); /* sync after groups of events */
		...
		pr_info("%s Entered synthesized logic but no event generated event cnt:%d masking events:%d\n",
		        __func__, f11->synth_events_sent, f11->mask_events);
		return;
	}
