...
static void gpio_keys_report_event(struct gpio_button_data *bdata, gpio_key_caller_t caller)
{
	...
	if (button->goog.prev_state == state && caller == GPIO_KEYS_WORK
		...
		/* Synthesize an in input event here because we are here due
		   ...
		   We only want to send one synthesized event per suspend
		   resume cycle.  Since we are assuming we were in a suspend
		   state we violate the button debounce times, and we could
		   be hitting multiple erroneous interrupts generated by
		   the camera button. */
		state = !state;
...
static int gpio_keys_resume(struct device *dev)
{
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (button->wakeup && device_may_wakeup(&pdev->dev)) {
			...
			/* Clear gpio button synthesize flag */
			button->goog.synth_sent = 0;
		}
