...
static ssize_t attr_addr_set(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct lsm303dlhc_mag_data *gyro = dev_get_drvdata(dev);
	unsigned long val;
...
static int lsm303dlhc_mag_remove(struct i2c_client *client)
{
	...
	#ifdef DEBUG
	pr_info(KERN_INFO "LSM303DLHC driver removing\n");
	#endif
	lsm303dlhc_mag_input_cleanup(mag);
	lsm303dlhc_mag_device_power_off(mag);
...
static int lsm303dlhc_mag_suspend(struct device *dev)
{
	...
	#if DEBUG
	pr_info(KERN_INFO "lsm303dlhc_suspend\n");
	#endif
	/* TO DO */
	#endif
	return 0;
...
static int lsm303dlhc_mag_resume(struct device *dev)
{
	...
	struct lsm303dlhc_data *mag = i2c_get_clientdata(client);
	#if DEBUG
	pr_info(KERN_INFO "lsm303dlhc_resume\n");
	#endif
	/* TO DO */
	#endif
	return 0;
...
{
	pr_info(KERN_INFO "lsm303dlhc magnetometer driver\n");
